...
- **隔离测试数据库**：每次运行使用独立的测试数据库，测试间互不影响
- **自动清理**：测试完成后自动清理测试数据
- **完整的CRUD操作覆盖**：验证创建、读取、更新、删除操作
- **边界情况测试**：测试无效输入、重复操作等边界情况
- **数据库一致性验证**：确保HTTP操作正确反映在数据库中
- **认证与权限验证**：测试所有认证和权限检查
- **JSON响应验证**：验证返回的JSON结构和状态码
